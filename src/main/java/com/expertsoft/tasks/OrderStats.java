...
class OrderStats {
        ...
        return customers
                .collect(Collectors.toMap(Customer::getEmail,
                        customer -> customer.getOrders().stream()
                                .map(card -> card.getPaymentInfo().getCardNumber())
                                .distinct().count()));
    }
    ...
    static Optional<String> mostPopularCountry(final Stream<Customer> customers) {
        return customers
                .collect(groupingBy(countries -> countries.getAddress().getCountry()))
                .entrySet().stream()
                .sorted(Map.Entry.<String, List<Customer>>comparingByValue(Comparator.comparingInt(List::size))
                        .reversed()
    ...
    static BigDecimal averageProductPriceForCreditCard(final Stream<Customer> customers, final String cardNumber) {
        ...
        return customers
                ...
                .flatMap(item -> IntStream.rangeClosed(1, item.getQuantity()).mapToObj(e -> item.getProduct()))
